...
The system is complete and deployed. An ASP.NET Core 9 Minimal API backed by EF Core/SQL Server serves a Vite React TypeScript SPA; both run on Azure ([live URLs](#deploy-to-azure)). The `Auth` module exposes register/login/refresh/logout/me behind role-based JWT authorization, the `Resources` module exposes room/schedule browsing plus admin CRUD, `GET /api/v1/admin/bookings` gives admins the cross-user booking list, `POST /api/v1/bookings` guarantees exactly one `201 Created` and every other request a `409 slot_already_booked` under concurrent load (see [ADR 0002](docs/architecture/0002-booking-concurrency.md)), and the `Realtime` module pushes a `SlotBookingChanged` event — through Azure SignalR Service in production — to everyone viewing that room the instant a booking commits, so every open client sees the status change without refreshing.
...
1. Open the [frontend](#deploy-to-azure) and click **Create one** on the sign-in screen — self-service sign-up always grants the `User` role, never `Admin`.
2. Pick a room. Every environment is seeded with demo rooms (`Falcon`, `Phoenix`) and a rolling three-day window of hourly slots, so the schedule is never empty.
3. To see the concurrency guarantee as a user sees it, open the same room in a second browser (or a private window) signed in as a different account and book the same slot from both: one gets the booking, the other a "The time slot is already booked." conflict, and both schedules flip to **Booked** without a refresh. The automated proof of the same invariant is [`CreateBooking_ConcurrentRequestsForSameSlot_ExactlyOneSucceeds`](tests/Backend.IntegrationTests/BookingEndpointsTests.cs).
4. Admin credentials for the deployed environment are generated by `infra/provision.sh` and stored only in the Web App's application settings — never in this repository. They are supplied with the submission; signing in with them adds **Manage rooms** and **All bookings** to the header.

The first request after a quiet period cold-starts the free-tier backend (see [Deploy to Azure](#deploy-to-azure)) and can take 10-30 seconds.
...
| Frontend | Vite, React, TypeScript, React Router, TanStack Query, MUI, SignalR client — sign-up/sign-in, room list, live room schedule, admin room/slot management and cross-user booking list |
...
The feature boundaries and deployment model are described in [ADR 0001: System architecture](docs/architecture/0001-system-architecture.md). The booking race-condition strategy is described in [ADR 0002: Booking concurrency](docs/architecture/0002-booking-concurrency.md).
...
`dotnet run` in the `Development` environment automatically applies pending migrations on startup; every other environment gets `dotnet ef database update` from the deploy workflow instead. Seeding runs in *all* environments and is idempotent: the `User`/`Admin` roles, the configured admin account, and the demo rooms `Falcon`/`Phoenix` with a rolling three-day window of hourly slots. That window is topped up on every start, so neither a local database nor the deployed one goes stale or greets a reviewer with an empty room list. Integration tests never touch the `docker-compose` container: they start and dispose their own SQL Server via Testcontainers.
...
`Jwt:SigningKey` and `Admin:Email`/`Admin:Password` in `appsettings.Development.json` are tracked dev-only defaults, same convention as the SQL SA password — `dotnet run` bootstraps a working admin account (`admin@meetingroombooking.local`) with zero extra setup. An administrator is never created from client input: `POST /api/v1/auth/register` always assigns `User`, and the only path to `Admin` is `AdminSeeder`, which reads `Admin:Email`/`Admin:Password` from configuration. In production those are app settings generated by `infra/provision.sh`, so provisioning an admin stays a deliberate deploy-time act.
...
## Bookings across users (admin)

`GET /api/v1/admin/bookings` (`Admin` only) is the task's "view all bookings across users" surface. It returns bookings ordered by slot start, denormalized with the room name, the slot window, and the booking user's email; `resourceId`, `fromUtc`, and `toUtc` filter, and `skip`/`take` page (100 per page, also the hard cap — out-of-range values are clamped, never rejected). The SPA renders it at `/admin/bookings`. It is read-only: V1 has no cancellation (ADR 0002).
...
No event is ever published for a losing/rolled-back booking attempt — `BookingEndpoints` only calls the notifier after `transaction.CommitAsync()` succeeds. The SPA's `RealtimeProvider` keeps one hub connection for the whole session (`accessTokenFactory` supplies the bearer token, since browsers can't set headers on a WebSocket handshake) with automatic reconnect; `useResourceSlotsRealtime` patches the specific slot in the TanStack Query cache on the event and refetches the whole schedule on reconnect (ADR 0001: "SignalR is a freshness channel, not the source of truth"). Locally this is the plain ASP.NET Core hub; when `Azure:SignalR:ConnectionString` is configured — as it is on the deployed Web App — the same hub is served through Azure SignalR Service, which is a DI registration in `Program.cs` and no change to hub or client code.

## Frontend routes

| Route | Access | Purpose |
| --- | --- | --- |
| `/login`, `/register` | Anonymous | Sign in; self-service sign-up, always as `User` |
| `/resources` | Signed in | Room list |
| `/resources/{id}` | Signed in | Live schedule and booking; admins additionally add and delete slots here |
| `/admin/resources` | `Admin` | Create, rename, and delete rooms |
| `/admin/bookings` | `Admin` | Every booking across every user |
| `/health` | Anonymous | API connectivity check |

Admin-only routes redirect a regular user back to `/resources`; the client-side check is a UX convenience, and every request is re-authorized server-side by role.
...
Development was carried out with Claude Code, and the repository carries its configuration:

- `CLAUDE.md` — the project brief every session starts from: structure, binding conventions (the concurrency invariant, module boundaries, CORS rules), and the verification checklist.
- `.claude/settings.json` — tracked permissions: the routine build/test/format commands are pre-approved, while `.env`/local app-settings reads and credential-creating `az` calls are denied.
- `.claude/skills/verify/SKILL.md` — a `/verify` skill that runs the checklist below in CI order, checks Docker first (so the integration tests can never be silently skipped), and reports the failing command verbatim.

Commits are atomic, each describing what changed and why, and are co-authored by the model that wrote them.
